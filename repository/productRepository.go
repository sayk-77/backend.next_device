...
func (pr *ProductRepository) GetFilteredLaptops(
	priceFrom, priceTo *int,
	brands []string,
	screenFrom, screenTo *float64,
	memories, ram, cpuTypes, gpuTypes []string,
	limit, offset int,
) ([]*models.Products, error) {

	fmt.Printf("Фильтрация ноутбуков\n")
	...
	fmt.Printf("Память: %v\n", memories)
	fmt.Printf("Оперативная память: %v\n", ram)
	fmt.Printf("Тип процессора: %v\n", cpuTypes)
	fmt.Printf("Тип графического процессора: %v\n", gpuTypes)
	fmt.Printf("Лимит: %d, Смещение: %d\n", limit, offset)

	query := pr.db.Model(&models.Products{}).
		Joins("JOIN categories ON products.category_id = categories.id").
		Joins("JOIN brands ON products.brand_id = brands.id").
		Joins("JOIN laptop_filters ON products.id = laptop_filters.laptop_id").
	...
	if screenFrom != nil {
		query = query.Where("laptop_filters.screen_size >= ?", *screenFrom)
	}
	if screenTo != nil {
		query = query.Where("laptop_filters.screen_size <= ?", *screenTo)
	}

	if len(memories) > 0 {
		query = query.Where("laptop_filters.storage IN (?)", memories)
	}
	if len(ram) > 0 {
		query = query.Where("laptop_filters.ram IN (?)", ram)
	}
	if len(cpuTypes) > 0 {
		query = query.Where("laptop_filters.processor_brand IN (?)", cpuTypes)
	}
	if len(gpuTypes) > 0 {
		query = query.Where("laptop_filters.graphics_card_type IN (?)", gpuTypes)
	}
	...
	if len(products) == 0 {
		fmt.Println("Не найдено ноутбуков с указанными фильтрами.")
	} else {
		fmt.Printf("Найденные ноутбуки: %+v\n", products)
	}
