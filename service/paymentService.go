...
func (s *PaymentService) CreateCheckoutSession(orderID uint, totalPrice float64) (*stripe.CheckoutSession, error) {
	params := &stripe.CheckoutSessionParams{
		...
		SuccessURL: stripe.String("https://next-device.publicvm.com/success?order_id=" + strconv.Itoa(int(orderID)) + "&session_id={CHECKOUT_SESSION_ID}"),
		CancelURL:  stripe.String("https://next-device.publicvm.com/error?order_id=" + strconv.Itoa(int(orderID)) + "&session_id={CHECKOUT_SESSION_ID}"),
